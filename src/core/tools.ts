...
import { buildFreedcampAuthParams } from "../freedcamp.js"; // Adjusted path
...
async function executeFreedcampRequest(url: string, method: string, authParams: Record<string, string>, bodyData?: Record<string, any>) {
  ...
  // console.log is redirected to stderr in cli.ts and http-server.ts
...
export function registerToolHandlers(server: Server) {
  // Register available tools (ListTools)
  ...
  // Handle tool execution (CallTool)
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (!process.env.FREEDCAMP_API_KEY || !process.env.FREEDCAMP_API_SECRET || !process.env.FREEDCAMP_PROJECT_ID) {
        console.error("Missing Freedcamp API credentials or Project ID in environment variables.");
        return { content: [{ type: "text", text: "Error: Missing Freedcamp API credentials or Project ID. Please check server configuration." }] };
    }
    ...
    if (request.params.name === "freedcamp_add_task") {
      try {
        ...
        for (const taskArg of parsedArgs.tasks) {
    ...
    if (request.params.name === "freedcamp_list_tasks") {
      try {
        listTasksSchema.parse(arguments_); // Validate (empty) args
        const listAuthParams = buildFreedcampAuthParams({ // Renamed to avoid conflict
          api_key: process.env.FREEDCAMP_API_KEY!,
          api_secret: process.env.FREEDCAMP_API_SECRET!,
        });
        ...
        console.log("Freedcamp API response:", json);
        if (!resp.ok || (json && json.http_code >= 400)) {
          return { content: [{ type: "text", text: `Error: ${json?.msg || resp.statusText}`, details: json }] };
        }
        const tasks = json?.data?.tasks || [];
        return { content: [{ type: "text", text: JSON.stringify(tasks, null, 2) }] };
      } catch (err: any) {
        console.error("Error listing tasks:", err);
        return { content: [{ type: "text", text: `Request failed: ${err.message}`, details: JSON.stringify(err) }] };
      }
    }
